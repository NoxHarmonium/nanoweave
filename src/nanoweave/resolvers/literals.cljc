(ns nanoweave.resolvers.literals
  (:require [nanoweave.ast.literals #?@(:cljs [:refer [IdentiferLit StringLit FloatLit BoolLit NilLit ArrayLit TypeLit PairLit ObjectLit]])]
            [nanoweave.resolvers.base :refer [safe-resolve-value]]
            [nanoweave.resolvers.errors :refer [throw-resolve-error]]
            #?(:clj [nanoweave.java-interop :as j])
            [clojure.string :as string]
            [nanoweave.ast.base :refer [Resolvable]])
  #?(:clj (:import [nanoweave.ast.literals IdentiferLit StringLit FloatLit BoolLit NilLit ArrayLit TypeLit PairLit ObjectLit])))
...
(extend-protocol Resolvable
  ...
  (resolve-value [this input]
    (let [key (:value this)]
      #?(:clj
         (let [is-static (:static-prefix this)
               resolved-value
               (cond
                 (nil? input) input
